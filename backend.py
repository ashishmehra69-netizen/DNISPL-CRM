        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS opportunities (
                    ...
                    proposal_on_time TEXT,
                    revision_count INTEGER DEFAULT 0,
                    boq_accuracy NUMERIC DEFAULT 0,
            ...
            cur.execute("ALTER TABLE opportunities ADD COLUMN IF NOT EXISTS proposal_submitted_at TEXT;")
            cur.execute("ALTER TABLE opportunities ADD COLUMN IF NOT EXISTS proposal_on_time TEXT;")
            cur.execute("ALTER TABLE opportunities ADD COLUMN IF NOT EXISTS revision_count INTEGER DEFAULT 0;")
            cur.execute("ALTER TABLE opportunities ADD COLUMN IF NOT EXISTS boq_accuracy NUMERIC DEFAULT 0;")
            cur.execute("ALTER TABLE opportunities ADD COLUMN IF NOT EXISTS poc_count INTEGER DEFAULT 0;")
            cur.execute("ALTER TABLE opportunities ADD COLUMN IF NOT EXISTS demo_count INTEGER DEFAULT 0;")
            cur.execute(
                """
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            opp_all = """
                SELECT id, name, account_id, value, stage, owner, sales_owner, workflow_stage,
                       ...
                       costing_tat, final_pricing_proposal, proposal_submitted_at, proposal_on_time, revision_count, boq_accuracy, poc_count, demo_count, presales_assigned_at, presales_due_at,
...
def upsert_opportunity():
    ...
    payload = {
        ...
        "proposal_submitted_at": (data.get("proposal_submitted_at") or data.get("proposalSubmittedAt") or "").strip(),
        "proposal_on_time": (data.get("proposal_on_time") or data.get("proposalOnTime") or "").strip(),
        "revision_count": int(data.get("revision_count") or data.get("revisionCount") or 0),
        "boq_accuracy": float(data.get("boq_accuracy") or data.get("boqAccuracy") or 0),
        "poc_count": int(data.get("poc_count") or data.get("pocCount") or 0),
        "demo_count": int(data.get("demo_count") or data.get("demoCount") or 0),
            ...
            if exists:
                cur.execute(
                    ...
                    SET name=%s, account_id=%s, value=%s, stage=%s, owner=%s, sales_owner=%s, workflow_stage=%s,
                        ...
                        costing_tat=%s, final_pricing_proposal=%s, proposal_submitted_at=%s, proposal_on_time=%s, revision_count=%s, boq_accuracy=%s, poc_count=%s, demo_count=%s, presales_assigned_at=%s, presales_due_at=%s,
                    ...
                    (
                        ...
                        payload["proposal_submitted_at"], payload["proposal_on_time"], payload["revision_count"], payload["boq_accuracy"], payload["poc_count"], payload["demo_count"],
